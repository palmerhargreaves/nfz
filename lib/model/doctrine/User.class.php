...
class User extends BaseUser
{
    ...
    function isImporterByGroup() {
        return $this->getGroupId() == self::IMPORTER_ID;
    }
    ...
    function getDealer()
    {
        ...
        foreach(DealerUserTable::getInstance()->createQuery('du')->where('user_id = ?', $this->getId())->innerJoin('du.Dealer d')->orderBy('d.dealer_type ASC')->execute() as $dealer_user_item) {
            if ($dealer_user_item->getDealer()->getDealerType() == Dealer::TYPE_NFZ || $dealer_user_item->getDealer()->getDealerType() == Dealer::TYPE_NFZ_PKW) {
    ...
    function getDealerUploadPath()
    {
        ...
        return sfConfig::get('app_dealer_upload_path') . '/' . $dealer->getNumber();
    }
    ...
    function getDealerDiscussion()
    {
        ...
        if ($dealer) {
            return DealerDiscussionTable::getInstance()->findDiscussion($dealer);
        }
    ...
    {
        $res = UserDealersTable::getInstance()
            ->createQuery()
            ->delete()
        ...
        //$this->link('AllowedDealers', array_values($dealers));
    ...
    function hasDealersListFromNaturalPerson() {
        if (!is_null($this->getId())) {
            ...
            $natural_person = NaturalPersonTable::getInstance()->createQuery()->where('regional_manager_nfz_id = ?', $this->getId())->fetchOne();

            if ($natural_person) {
                $res = array();

                $dealers_result = DealerTable::getInstance()->createQuery('d')->select('d.id')->where('nfz_regional_manager_id = ?', array($natural_person->getId()))->andWhere('dealer_type = ? or dealer_type = ?', array(Dealer::TYPE_NFZ, Dealer::TYPE_NFZ_PKW))->execute();
