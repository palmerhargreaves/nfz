...
class AgreementModelReport extends BaseAgreementModelReport
{
    ...
    function inFavoritesFile($fileId)
    {
        return AgreementModelReportFavoritesTable::getInstance()
            ->createQuery()
            ->where('report_id = ? and file_id = ?', array($this->getId(), $fileId))->count() > 0 ? true : false;
    }
    ...
    public function getUploadedFilesList($by_type) {
        return AgreementModelReportFilesTable::getUploadedFilesListBy($this->getModel()->getId(), self::UPLOADED_FILE_REPORT, $by_type);
    }
