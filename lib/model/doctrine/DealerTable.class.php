...
class DealerTable extends Doctrine_Table
{
    ...
    {
        $query = self::getInstance()->createQuery('d')->select('*')->where('importer_id = ?', array(1));
        self::queryByNumber($query);

        if (($limit == null) or ($offset == null))
            return $query->andWhere('dealer_type IN (1,3)')->orderBy('number');
        else
            return $query->andWhere('dealer_type IN (1,3)')->offset($offset)->limit($limit)->orderBy('number');
    }
     ...
     * @return Doctrine_Query
     */
    static function getVwDealersPkwNfzQuery($limit = null, $offset = null)
    {
        $query = self::getInstance()->createQuery('d')->select('*')->where('importer_id = ?', array(1));
        self::queryByNumber($query);

        if (($limit == null) or ($offset == null))
            return $query->andWhere('dealer_type IN (2,3)')->orderBy('number');
        else
    ...
    {
        $query = self::getInstance()->createQuery('d')->select('*')
